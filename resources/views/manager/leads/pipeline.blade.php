<x-app-layout>
    ...
    <div class="lp-shell px-2 sm:px-0">
        ...
        <section class="space-y-6" x-data="{ openStage: '{{ $statuses[0] ?? 'new' }}' }">
            @foreach ($statuses as $status)
                ...
                <article class="lp-card min-h-[72vh] p-5 sm:p-6">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <button
                            type="button"
                            class="rounded-lg border border-[var(--lp-border)] px-3 py-1.5 text-xs font-medium lp-title hover:bg-[var(--lp-canvas)]"
                            @click="openStage = openStage === '{{ $status }}' ? '' : '{{ $status }}'"
                            :aria-expanded="openStage === '{{ $status }}'"
                        >
                            <span x-text="openStage === '{{ $status }}' ? 'Collapse' : 'Expand'"></span>
                        </button>
                    </div>

                    <p class="mt-1 text-sm lp-muted">Newest to oldest, with five rows shown by default.</p>

                    <div class="mt-4" x-show="openStage === '{{ $status }}'" x-cloak>
